...
cohort_period.py
================
Constrained additive GP model for cohort + period effects:

    y_i = [mu] + alpha_{c(i)} + beta_{t(i)} + eps_i
    eps_i ~ N(0, sn^2)

Both alpha and beta are represented in sum-to-zero reduced spaces:
    alpha = Q_drop_c @ z_c
    beta  = Q_drop_t @ z_t

Inference uses MAP over hyperparameters (and optional mu) followed by a Laplace
approximation and Monte Carlo uncertainty propagation
"""
...
class CohortPeriodModel:
    """
    Additive constrained GP model (cohort + period), with optional global mean.

    Default use_global_mean=False.
    """
    ...
    def __init__(
        ...
        self.hyperparams_posterior_: Optional[dict] = None
        self.posterior_samples_: Optional[dict] = None
        ...
        self._obs_c: Optional[np.ndarray] = None
    ...
    def fit(
        ...
        n = len(y_np)
        if n == 0:
            raise ValueError("Empty observations provided to CohortPeriodModel.fit.")
        ...
        def marginal_loglik_cp_constrained(
            ...
            sigma_y = h_tilde_t @ k_z @ h_tilde_t.T + (sn ** 2 + jitter) * torch.eye(n, dtype=torch.float64)
            try:
                l = torch.linalg.cholesky(sigma_y)
            except RuntimeError:
                sigma_y = sigma_y + 1e-4 * torch.eye(n, dtype=torch.float64)
        ...
        log_ell_c = torch.tensor(math.log(1.0), requires_grad=True, dtype=torch.float64)
        log_sf_c = torch.tensor(math.log(sf_init), requires_grad=True, dtype=torch.float64)
        log_ell_t = torch.tensor(math.log(1.0), requires_grad=True, dtype=torch.float64)
        log_sf_t = torch.tensor(math.log(sf_init), requires_grad=True, dtype=torch.float64)
        log_sn = torch.tensor(math.log(sn_init), requires_grad=True, dtype=torch.float64)

        if self.use_global_mean:
            params = [mu_raw, log_ell_c, log_sf_c, log_ell_t, log_sf_t, log_sn]
            log_params = [log_ell_c, log_sf_c, log_ell_t, log_sf_t, log_sn]
        else:
            params = [log_ell_c, log_sf_c, log_ell_t, log_sf_t, log_sn]
            log_params = params
        ...
        self.bounds_used_ = {
            ...
            "sn": [float(bounds[4][0]), float(bounds[4][1])],
            "y_scale": float(y_scale),
        }

        def log_prior_hyper(lps: list[torch.Tensor]) -> torch.Tensor:
            # weakly-informative Gaussian priors in log-space
        ...
        def closure() -> torch.Tensor:
            ...
            if self.use_global_mean:
                mu = mu_raw
                y_centered = y_t - mu
                lp = log_prior_hyper(log_params) + log_prior_mu(mu)
            else:
                y_centered = y_t
                lp = log_prior_hyper(log_params)
            ll = marginal_loglik_cp_constrained(y_centered, vals[0], vals[1], vals[2], vals[3], vals[4])
            loss = -(ll + lp)
            loss.backward()
            return loss
        ...
        mu_map = float(mu_raw.detach()) if self.use_global_mean else 0.0

        # Laplace over optimized params
        for p in params:
        ...
        if self.use_global_mean:
            ...
            nll = -(marginal_loglik_cp_constrained(y_centered_now, vals_now[0], vals_now[1], vals_now[2], vals_now[3], vals_now[4])
                    + log_prior_hyper(log_params) + log_prior_mu(mu_raw))
        else:
            nll = -(marginal_loglik_cp_constrained(y_t, vals_now[0], vals_now[1], vals_now[2], vals_now[3], vals_now[4])
        ...
        for s in range(self.n_laplace_samples):
            ...
            mu_red_s, sigma_red_s = _analytical_gp_posterior(y_t - mu_s, h_tilde_t, k_z, vals_s[4])
            mu_red_samps.append(mu_red_s)
            sigma_red_samps.append(sigma_red_s)
            mu_draws.append(mu_s)

        # Natural-scale Laplace samples for hyperparameter posterior summaries/plots.
        if self.use_global_mean:
            lp_draws = draws[:, 1:]
            mu_draw_np = draws[:, 0].detach().cpu().numpy()
        else:
            lp_draws = draws
            mu_draw_np = np.zeros(self.n_laplace_samples, dtype=float)

        ell_c_s = torch.exp(lp_draws[:, 0]).clamp(bounds[0][0], bounds[0][1]).detach().cpu().numpy()
        sf_c_s = torch.exp(lp_draws[:, 1]).clamp(bounds[1][0], bounds[1][1]).detach().cpu().numpy()
        ell_t_s = torch.exp(lp_draws[:, 2]).clamp(bounds[2][0], bounds[2][1]).detach().cpu().numpy()
        sf_t_s = torch.exp(lp_draws[:, 3]).clamp(bounds[3][0], bounds[3][1]).detach().cpu().numpy()
        sn_s = torch.exp(lp_draws[:, 4]).clamp(bounds[4][0], bounds[4][1]).detach().cpu().numpy()
        ell_s = 0.5 * (ell_c_s + ell_t_s)
        sf_s = 0.5 * (sf_c_s + sf_t_s)

        mu_red_stack = torch.stack(mu_red_samps, dim=0)  # [S, dc+dt]
        sigma_red_stack = torch.stack(sigma_red_samps, dim=0)  # [S, dc+dt, dc+dt]
        ...
        self.hyperparams_posterior_ = {
            "mean": {
                "ell": float(np.mean(ell_s)),
                "sf": float(np.mean(sf_s)),
                "sn": float(np.mean(sn_s)),
                "ell_c": float(np.mean(ell_c_s)),
                "sf_c": float(np.mean(sf_c_s)),
                "ell_t": float(np.mean(ell_t_s)),
                "sf_t": float(np.mean(sf_t_s)),
            },
            "sd": {
                "ell": float(np.std(ell_s, ddof=1)),
                "sf": float(np.std(sf_s, ddof=1)),
                "sn": float(np.std(sn_s, ddof=1)),
                "ell_c": float(np.std(ell_c_s, ddof=1)),
                "sf_c": float(np.std(sf_c_s, ddof=1)),
                "ell_t": float(np.std(ell_t_s, ddof=1)),
                "sf_t": float(np.std(sf_t_s, ddof=1)),
            },
        }
        ...
        self._n_cohorts = c
        self._n_periods = t
        self._obs_c = obs_c_np
        self._obs_t = obs_t_np
    ...
    def predict(self, obs_c: "array-like", obs_t: "array-like") -> np.ndarray:
        ...
        return self.mu_ + self.alpha_[c_idx] + self.beta_[t_idx]
    ...
    def results_dict(self) -> dict:
        ...
        return {
            ...
            "hyperparams_posterior": self.hyperparams_posterior_,
    ...
    def summary(self) -> None:
        ...
        print(
            f"\n{'-' * 60}\n"
            f"  CohortPeriodModel ({'with' if self.use_global_mean else 'without'} global mean)\n"
            f"{'-' * 60}\n"
